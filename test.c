...
#define TEST_LITERAL(expected_type, json)\
    do{\
        lept_value v;\
        lept_init(&v);\
        lept_set_boolean(&v, expected_type == LEPT_FALSE);\
...
static void test_parse_number()
{
    ...
    TEST_NUMBER( 4.9406564584124654e-324, "4.9406564584124654e-324"); /* Min subnormal double */
...
static void test_parse_array()
{
    ...
    for(i = 0; i != 4; ++i) {
        lept_value *element = lept_get_array_element(&v, i);
        EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(element));
        EXPECT_EQ_SIZE_T(i, lept_get_array_size(element));
        for(j = 0; j != i; ++j) {
            EXPECT_EQ_INT(LEPT_NUMBER, lept_get_type(lept_get_array_element(element, j)));
            EXPECT_EQ_DOUBLE((double)j, lept_get_number(lept_get_array_element(element, j)));
        }
    }
    lept_free(&v);
}
...
static void test_access_array()
{
    ...
    for (j = 0; j <= 5; j += 5) {
        ...
        for (i = 0; i < 10; i++) {
            lept_set_number(lept_pushback_array_element(&a), i);
        }
    ...
    for (i = 0; i < 2; i++) {
        lept_set_number(lept_insert_array_element(&a, i), i);
    }
    EXPECT_EQ_SIZE_T(8, lept_get_array_size(&a));
    for (i = 0; i < 8; i++) {
        EXPECT_EQ_DOUBLE((double)i, lept_get_number(lept_get_array_element(&a, i)));
    ...
    lept_set_string(lept_pushback_array_element(&a), "Hello", 5);
...
static void test_access_object()
{
    ...
    for (j = 0; j <= 5; j += 5) {
        ...
        for (i = 0; i < 10; i++) {
            char key[2] = "a";
            key[0] += i;
            lept_set_number(lept_set_object_value(&o, key, 1), i);
        }
    ...
    lept_set_string(lept_set_object_value(&o, "World", 5), "Hello", 5);
